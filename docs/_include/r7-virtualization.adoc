...
----
export VM_NAME="ocp-bastion" KS_URL="http://rhv2.lab.linuxsoup.com/ks.cfg"

virt-install \
    --ram=16000 --location=/var/lib/libvirt/images/rhel-8.1-x86_64-dvd.iso \
    --disk /var/lib/libvirt/images/${VM_NAME}.qcow2,bus=virtio,sparse=no,size=25 \
...
virt-install \
    ...
    --graphics vnc \
    --ram 16000 \
    --location /var/lib/libvirt/images/rhel-8.1-x86_64-dvd.iso \
    --name ${VM_NAME} \
    --extra-args "ks=${KS_URL} ip=192.168.1.160 netmask=255.255.255.0 dns=192.168.1.30 gateway=192.168.1.1"
----

Some simple virtual machine management commands
