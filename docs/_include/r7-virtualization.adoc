...
In this example, we are creating a host called "ocp-bastion"
...
virt-install --name=${VM_NAME} --extra-args="ks=${KS_URL}" --ram=16000 --location=/var/lib/libvirt/images/rhel-8.1-x86_64-dvd.iso  --disk /var/lib/libvirt/images/${VM_NAME}.qcow2,bus=virtio,sparse=no,size=25 --console pty,target_type=virtio --graphics vnc
----
