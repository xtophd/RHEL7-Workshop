...
.[root@workstation]#
----
for i in {1..50} ; do mktemp -d /mnt/exercise4/XXXXXX ; done
----
...
The prevoius command should have copied approximately 100MB in 50 folders yielding about 5G of traditional fielsystem consumption.
...
If you wait and rerun the `df` and `vod-stats` commands, what you will notice is that the space is recovered as VDO detects duplicate blocks and de-dups the data in the background.  It may take a few minutes, but the available filestem capacity should once approach 27G.
